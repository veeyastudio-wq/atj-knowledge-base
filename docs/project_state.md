...
API layer: scripts/api.py is a FastAPI wrapper around the orchestration loop, exposing POST /chat (accepts user_id and message, returns the model response plus compliance pass/fail, fallback triggered flag, memory facts retrieved, and session_id) and GET /health. Calls the same functions in the same sequence as the CLI harness: memory retrieval, RAG retrieval, Claude API, compliance check, memory write. Local development only, not deployed. Run with uvicorn against the same atj-db and atj-neo4j instances as the CLI harness.
